...
(define (better-reverse items)
  (b-reverse-helper items '()))
;使用了一个临时表， 形成了尾递归， 是比append更快更好的方法。
(define (b-reverse-helper items res)
  (if (null? items)
      res
      (b-reverse-helper (cdr items) (cons (car items) res))))
